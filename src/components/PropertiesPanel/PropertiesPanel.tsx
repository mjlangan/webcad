...
function NumField({ label, value, step = 0.01, min, onChange }: NumFieldProps) {
  return (
    ...
    case 'box':
      return (
        <Section title="Geometry — Box">
          <NumField label="W" value={geometry.width}  onChange={(v) => onUpdate({ ...geometry, width:  v })} />
          <NumField label="H" value={geometry.height} onChange={(v) => onUpdate({ ...geometry, height: v })} />
          <NumField label="D" value={geometry.depth}  onChange={(v) => onUpdate({ ...geometry, depth:  v })} />
        </Section>
      );
    case 'sphere':
      return (
        <Section title="Geometry — Sphere">
          <NumField label="Radius"   value={geometry.radius}         onChange={(v) => onUpdate({ ...geometry, radius:         v })} />
    ...
    case 'cylinder':
      return (
        <Section title="Geometry — Cylinder">
          <NumField label="R Top"    value={geometry.radiusTop}    onChange={(v) => onUpdate({ ...geometry, radiusTop:    v })} />
          <NumField label="R Bottom" value={geometry.radiusBottom} onChange={(v) => onUpdate({ ...geometry, radiusBottom: v })} />
          <NumField label="Height"   value={geometry.height}       onChange={(v) => onUpdate({ ...geometry, height:       v })} />
    ...
    case 'torus':
      return (
        <Section title="Geometry — Torus">
          <NumField label="Radius"    value={geometry.radius}          onChange={(v) => onUpdate({ ...geometry, radius:          v })} />
          <NumField label="Tube"      value={geometry.tube}            onChange={(v) => onUpdate({ ...geometry, tube:            v })} />
  ...
  return (
    <div className="props-panel">
      ...
      <Section title="Position">
        <NumField label="X" value={transform.position[0]} onChange={(v) => setPos(0, v)} />
        <NumField label="Y" value={transform.position[1]} onChange={(v) => setPos(1, v)} />
        <NumField label="Z" value={transform.position[2]} onChange={(v) => setPos(2, v)} />
      </Section>
      ...
      <Section title="Scale">
        <NumField label="X" value={transform.scale[0]} min={0.001} onChange={(v) => setScale(0, v)} />
        <NumField label="Y" value={transform.scale[1]} min={0.001} onChange={(v) => setScale(1, v)} />
        <NumField label="Z" value={transform.scale[2]} min={0.001} onChange={(v) => setScale(2, v)} />
      </Section>
